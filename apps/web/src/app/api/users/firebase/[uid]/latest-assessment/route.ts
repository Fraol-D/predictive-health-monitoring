...
export async function GET(
    ...
    try {
        ...
        const backendRes = await fetch(`${BACKEND_API_BASE_URL}/users/firebase/${uid}/latest-assessment`);

        if (!backendRes.ok) {
            const errorBody = await backendRes.json();
            return NextResponse.json({ error: errorBody.message || 'Failed to fetch latest assessment' }, { status: backendRes.status });
        }
